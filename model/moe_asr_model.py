...
class MoeAsrModel(torch.nn.Module):
    def __init__(self,
                 vocab_size: int,
                 encoder,
                 decoder,
    ...
    def forward(
        ...
        if self.domain_weight != 0.0:
            ...
            loss_domain = self.domain_classifier(domain_encoder_output, domain_id) * self.domain_weight
        else:
        ...
        if self.fusion is not None:
            encoder_output = self.fusion(encoder_output, domain_encoder_output)

        # Decoder
        if self.att_weight != 0.0:
            loss_att = self._calc_att_loss(encoder_output, encoder_mask, text, text_lengths) * self.att_weight
        else:
        ...
        return {"loss": loss, "loss_att": loss_att, "loss_ctc": loss_ctc, "loss_domain": loss_domain, "loss_aux": loss_aux}
    ...
    def _forward_encoder(
        ...
        if self.use_branch:
            encoder_output, encoder_mask, _ = self.encoder(speech, speech_lengths,
    ...
    def load_no_moe_checkpoint(self, load_path):
