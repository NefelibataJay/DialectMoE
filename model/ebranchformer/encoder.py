...
class EBranchformerLayer(nn.Module):
    ...
    def forward(
        ...
        # Feed forward module
        if self.feed_forward is not None:
            x = x + self.ff_scale * self.dropout(self.feed_forward(self.norm_ff(x)))
        x = self.norm_final(x)
        return x, mask
...
class EBranchformerEncoder(nn.Module):
    def __init__(self,
        ...
        self.encoders = nn.ModuleList([EBranchformerLayer(
            output_size,
    ...
    def forward(
        ...
        for layer in self.encoders:
            xs, chunk_masks = layer(xs, chunk_masks, pos_emb, mask_pad)
