...
class MAModel(torch.nn.Module):
    def __init__(self,
                 ...
                 decoder,
                 ctc,
                 domain_encoder: Optional[torch.nn.Module] = None,
                 domain_classifier: Optional[torch.nn.Module] = None,
                 fusion: Optional[torch.nn.Module] = None,
    ...
    def forward(
        ...
        if self.domain_encoder is not None:
            self.domain_weight = 0.1 if self.domain_weight == 0.0 else self.domain_weight
            per_encoder_output, mask = self.domain_encoder(encoder_input, mask)
        else:
            self.domain_weight = 0.0
            per_encoder_output = None

        if self.domain_weight != 0.0:
            assert domain_label is not None
            loss_domain = self.domain_classifier(per_encoder_output, domain_label) * self.domain_weight
        else:
            loss_domain = torch.tensor(0.0)

        # ASR Encoder
        if self.use_branch:
            asr_encoder_output, encoder_mask, aux_loss_collection = self.asr_encoder(encoder_input, mask,
                                                                                     per_encoder_output)
        else:
            assert per_encoder_output is not None
            asr_encoder_output, encoder_mask, aux_loss_collection = self.asr_encoder(per_encoder_output, mask,
                                                                                     per_encoder_output)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)

        if self.aux_scale is not None:
            loss_aux = self._calc_aux_loss(aux_loss_collection)
        else:
            loss_aux = torch.tensor(0.0)

        # Fusion
        if self.fusion is not None:
            asr_encoder_output = self.fusion(asr_encoder_output, per_encoder_output)

        # Decoder
        if self.att_weight != 0.0:
            loss_att = self._calc_att_loss(asr_encoder_output, encoder_mask, text, text_lengths) * self.att_weight
        else:
            loss_att = torch.tensor(0.0)

        if self.ctc_weight != 0.0:
            loss_ctc = self.ctc(asr_encoder_output, encoder_out_lens, text, text_lengths) * self.ctc_weight
        else:
            loss_ctc = torch.tensor(0.0)

        loss = loss_ctc + loss_att + loss_domain + loss_aux

        return {"loss": loss, "loss_att": loss_att, "loss_ctc": loss_ctc,
                "loss_domain": loss_domain, "loss_aux": loss_aux}
    ...
    def _calc_aux_loss(
            self,
            aux_loss_collection: List[Tuple[torch.Tensor, torch.Tensor]],
    ) -> torch.Tensor:
        loss_aux = torch.tensor(0.0,device=aux_loss_collection[0][0].device)
        for aux in aux_loss_collection:
            l1_loss, importance_loss = aux
            loss_aux += self.aux_scale[0] * l1_loss
            loss_aux += self.aux_scale[1] * importance_loss
        return loss_aux
    ...
    def _forward_encoder(
        ...
        if self.domain_encoder is not None:
            self.domain_weight = 0.1 if self.domain_weight == 0.0 else self.domain_weight
            per_encoder_output = self.domain_encoder(encoder_input, mask)
        else:
            self.domain_weight = 0.1
            per_encoder_output = encoder_input
        ...
        if self.fusion is not None:
            asr_encoder_output = self.fusion(asr_encoder_output, per_encoder_output)
        return asr_encoder_output, encoder_mask
    ...
    def recognize(
        ...
        # 1. Encoder
        encoder_out, encoder_mask = self._forward_encoder(speech, speech_lengths)  # (B, maxlen, encoder_dim)
    ...
    def _ctc_prefix_beam_search(
            ...
            simulate_streaming: bool = False,
            context_graph: ContextGraph = None,
    ) -> Tuple[List[Tuple[tuple, float]], Tensor]:
