...
def compute_fbank(data,
    ...
    for sample in data:
        ...
        yield dict(key=sample['key'], label=sample['label'], feat=mat, domain=sample['domain'])
...
def padding(data):
    ...
    for sample in data:
        ...
        padding_labels = pad_sequence(sorted_labels,
                                      batch_first=True,
                                      padding_value=-1)
        accent_labels = torch.tensor([x['domain'] for x in sample], dtype=torch.int64)

        yield (sorted_keys, padded_feats, padding_labels, feats_lengths,
               label_lengths, accent_labels)
