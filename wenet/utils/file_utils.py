...
def read_symbol_table(symbol_table_file):
    symbol_table = {}
    symbol_table_path = os.path.join(symbol_table_file, "lang_char.txt")
    with open(symbol_table_path, 'r', encoding='utf8') as fin:
        for line in fin:
...
def read_domain_table(symbol_table_file):
    domain_table = {}
    domain_table_path = os.path.join(symbol_table_file, "domain_table.txt")
    with open(domain_table_path, 'r', encoding='utf8') as fin:
        for line in fin:
            arr = line.strip().split()
            assert len(arr) == 2
            domain_table[arr[0]] = int(arr[1])
    return domain_table
